...
export default function KasKeluarPage() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      const response = await fetch(`/api/transactions?profileId=${activeProfile.id}&type=EXPENSE`);
      if (response.ok) {
  ...
  const fetchCategories = async () => {
    ...
    try {
      const response = await fetch(`/api/categories?profileId=${activeProfile.id}&type=EXPENSE`);
      if (response.ok) {
        const data = await response.json();
        setCategories(data);
      }
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[50vh]">
          <TrendingDown className="h-16 w-16 text-navy-900 mb-4" />
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-navy-900">Kas Keluar</h1>
            <p className="text-gray-600 mt-1">Catat dan kelola pengeluaran Anda</p>
          </div>
        ...
        <Card className="bg-gradient-to-br from-red-50 to-white border-red-200">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-red-900">Total Pengeluaran</CardTitle>
                <CardDescription className="text-red-700">Semua waktu</CardDescription>
              </div>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Ubah detail pengeluaran</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
