    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="flex flex-1">
        ...
        <main className={`flex-1 min-h-screen flex flex-col transition-all duration-300 ${sidebarOpen ? 'lg:ml-64' : 'lg:ml-0'}`}>
          {/* Header */}
          <header className={`sticky top-0 z-20 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 ${sidebarOpen ? 'lg:pl-0' : ''} pl-12`}>
            <div className="container mx-auto px-4 py-4 flex items-center justify-between">
              <h1 className={`text-2xl font-bold text-navy-900 ${sidebarOpen ? 'lg:hidden' : ''}`}>
                Wealth Tracker
              </h1>
